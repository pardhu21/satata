...
import migrations_satata.models as migrations_models
...
def get_migrations_not_executed(db: Session):
    try:
        ...
        db_migrations = (
            db.query(migrations_models.MigrationSatata).filter(migrations_models.MigrationSatata.executed == False).all()
        )
...
def set_migration_as_executed(migration_id: int, db: Session):
    try:
        ...
        db_migration = (
            db.query(migrations_models.MigrationSatata)
            .filter(migrations_models.MigrationSatata.id == migration_id)
            .first()
        )
